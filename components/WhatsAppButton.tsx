...
export default function WhatsAppButton({ locale }: { locale: Locale }) {
  ...
  return (
    <a
      ...
      className="group fixed bottom-5 right-5 z-40 block sm:bottom-7 sm:right-7"
    >
      <span className="pointer-events-none absolute inset-0 -z-10 animate-ping rounded-full bg-[#25D366]/60 opacity-60 group-hover:opacity-0" />
      <span className="flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_10px_25px_-5px_rgba(37,211,102,0.55)] ring-4 ring-white/70 transition duration-200 group-hover:scale-105 group-hover:shadow-[0_12px_30px_-5px_rgba(37,211,102,0.75)] dark:ring-ink-900/40">
        <svg
          viewBox="0 0 32 32"
          aria-hidden="true"
      ...
      <span className="pointer-events-none absolute right-full top-1/2 mr-3 -translate-y-1/2 whitespace-nowrap rounded-full bg-ink-900/95 px-3 py-1.5 text-xs font-semibold text-white opacity-0 shadow-lg transition duration-200 group-hover:opacity-100 group-hover:-translate-x-1 hidden sm:block">
        {LABEL[locale]}
      </span>
    </a>
  );
